...
# Новая функция для извлечения списка разделов из текста рекомендаций
from utils.parsers import extract_sections_from_recommendations
...
async def generate_report_async(
    ...
    try:
        ...
        # 3) [async] Получаем список разделов из файла рекомендаций
        logger.info("3) [async] Извлекаем список разделов из рекомендаций…")
        ...
        if len(doc.tables) < 3:
            raise ValueError("В шаблоне недостаточно таблиц для формирования списка разделов.")
        sections_table = doc.tables[2]
        # Убеждаемся, что таблица имеет достаточно строк (1 строка — заголовок)
        while len(sections_table.rows) < len(sections) + 1:
            sections_table.add_row()
        # Записываем актуальные названия разделов в первый столбец таблицы
        for i, sec in enumerate(sections, start=1):
            sections_table.rows[i].cells[0].text = sec

        # 5) Разбиение на секции (параллельно в пуле потоков)
